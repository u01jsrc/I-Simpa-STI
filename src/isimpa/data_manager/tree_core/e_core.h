		...
		if (noeudCourant != NULL)
		{
			...
			while (currentChild != NULL)
			{
				if (currentChild->GetAttribute("eid", &propValue))
				{
					...
					else if (typeEle == Element::ELEMENT_TYPE_CORE_SPPSAGH)
			...
			bool found = find_if(fils.begin(), fils.end(), [&](Element* el) {return el->GetElementInfos().typeElement == Element::ELEMENT_TYPE_CORE_SPPSAGH; }) != fils.end();
			if (!found)
