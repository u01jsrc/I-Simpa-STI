	...
	if(root)
	{
		...
		if(simuNode)
		{
			...
			if (advancedNode) 
			{
				...
				SetConfigInformation(IPROP_DIFFUSION_ORDER, advancedNode->GetProperty("diffusion_order").ToInt());
				SetConfigInformation(IPROP_SPECULAR_WHEN_REACHED, advancedNode->GetProperty("specular_when_order_reached").ToInt());
