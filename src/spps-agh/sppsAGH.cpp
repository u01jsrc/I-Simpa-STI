...
*	- ADDED: mean angle of incidence for receiver plane group calculation - required changes in I-SIMPA source code!
...
int MainProcess(int argc, char* argv[])
{
	...
	if (*configManager.FastGetConfigValue(Core_Configuration::IPROP_EXPORT_RECEIVER_INCIDENCE_ANGLE)) {
		cout << "Saving Ponctual Incidence Angle..." << endl;
		ReportManager::SaveReceiverIncidenceAngle("Punctual receiver incidence.gabe", threadsData, reportParameter);
