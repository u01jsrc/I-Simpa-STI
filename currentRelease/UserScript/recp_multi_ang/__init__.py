...
def GetMixedLevel(folderwxid,param,enable):
    ...
    #dans un tableau on place les indices des fichiers de donn�es des
    #r�cepteurs ponctuels
    recplist = folder.getallelementbytype(ui.element_type.ELEMENT_TYPE_REPORT_GABE)
    #Pour chaque r�cepteur on demande � l'application les donn�es trait�es du
    #fichier (niveau sonore et cumuls)
    for idrecp in recplist:
        ...
        if recp.getinfos()["name"] == "angle_stats":
            ...
            #if len(cols)==0: #si le tableau de sortie est vide alors on ajoute
            #les libell�s des lignes
            ...
            if len(cols2) == 0: #si le tableau de sortie est vide alors on ajoute les libell�s des lignes
                cols2.append(list(gridparam[0][0:])) #libell� Freq et Global
            if len(cols3) == 0: #si le tableau de sortie est vide alors on ajoute les libell�s des lignes
                cols3.append(list(gridparam[0][0:])) #libell� Freq et Global
